...
export default function useSelectAddress() {
	...
	const [barangaySelected, setBarangaySelected] = useState<BarangayType | null>(
		null
	);
	...
	useEffect(() => {
		if (regionSelected) {
			const selectedProvince = provinces.filter(
				(province) => regionSelected.region_code === province.region_code
			);
	...
	useEffect(() => {
		if (provinceSelected) {
			const selectedCity = cities.filter(
				(city) => provinceSelected.province_code === city.province_code
			);
	...
	useEffect(() => {
		if (citySelected) {
			const selectedBrgy = (barangays as BarangayType[]).filter(
				(brgy) => citySelected.city_code === brgy.city_code
			);
